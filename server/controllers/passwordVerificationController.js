	...
	try {
		...
		const updatedUserData = await User.findByIdAndUpdate(
			req.params.id,
			{
				...
				isVerified: false,
				unverifiedPassword: newPassword,
			},
			{ new: true }
...
router.get("/verification/:verificationCode", async (req, res) => {
	try {
		...
		user.password = user.unverifiedPassword;
		user.unverifiedPassword = "";
		...
		// Send response
		res.status(200).json({ message: "Password change verified successfully" });
	} catch (error) {
		console.error("Error verifying password change:", error);
		res.status(500).json({ error: "Failed to verify password change" });
	}
});
