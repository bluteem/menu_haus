<template>
	...
	<div class="sticky top-0 w-full bg-white shadow-md menu-nav px-8 py-6 flex overflow-x-auto">
		<a
			href="#"
			class="border-2 border-orange-500 rounded-xl focus:outline-none hover:bg-gray-200 transition duration-300 px-3 py-2 mr-2 flex items-center justify-center font-semibold min-w-48"
			>All Items</a
		>
		<a
			v-for="menuCategory in allMenuCategoriesData"
			:key="menuCategory._id"
			:href="'#' + toFriendlyURL(menuCategory.name)"
				...
				<li
					...
					<div class="flex-grow">
						...
						<p class="text-gray-600">
							Price: <span class="font-bold text-xl">${{ menuItem.price.toFixed(2) }}</span>
						</p>
						<div class="flex">
							<button
					...
					<div class="mt-2">
						...
						<div id="Carousel" class="relative" data-te-carousel-init data-te-ride="carousel">
							...
							<div class="relative w-full overflow-hidden after:clear-both after:block after:content-['']">
								<div
									v-for="(image, index) in selectedMenuItem.images"
									:key="index"
						...
						<p class="text-gray-600 mt-4">
							...
							{{
								selectedMenuItem.categoryInfo && selectedMenuItem.categoryInfo.length > 0
									? selectedMenuItem.categoryInfo[0].name
									: ""
							}}
						</p>
	...
	setup() {
		...
		const groupedMenuItems = computed(() => {
			...
			allMenuItemsData.value.forEach((menuItem) => {
				const categoryName =
					menuItem.categoryInfo && menuItem.categoryInfo.length > 0 ? menuItem.categoryInfo[0].name : "Uncategorized";
